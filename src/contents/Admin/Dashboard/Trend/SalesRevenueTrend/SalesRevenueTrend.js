import { useGetSalesRevenueTrends } from "@/api/services/reportServices/trendSercices";
...
const SalesRevenueTrend = () => {
  ...
  const { data: trendResponse, isLoading } = useGetSalesRevenueTrends(filters);
  ...
  const salesRevenueTrends = trendResponse?.data?.data || [];

  console.log({ salesRevenueTrends });

  return (
    <div>
      <p className="text-lg font-semibold my-2">Sales Revenue Trend</p>
      <AppBarChart
        categories={
          salesRevenueTrends?.map((rep) => moment(rep?.date).format("DD")) || []
        }
        series={[
          {
            name: "Sales",
            data:
              salesRevenueTrends?.map((rep) => rep?.totalSales?.toFixed(2)) ||
              [],
          },
          {
            name: "Revenue",
            data:
              salesRevenueTrends?.map((rep) => rep?.totalRevenue?.toFixed(2)) ||
              [],
          },
        ]}
      />
